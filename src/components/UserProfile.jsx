...
//       // // ❌ Logout if user doc doesn't exist OR role is not "user"
...
//       //     icon: 'warning',
//       //     title: 'Access Denied',
//       //     text: 'Please access your right dashboard',
//       //   });
//       //   return;
//       // }
...
// Reuse your existing styled-components here...
...
//   useEffect(() => {
//     const storedUserId = localStorage.getItem("user"); // This is just the user ID
...
//             setPhone(data.phone || '');
//           } else {
//             Swal.fire("Not Found", "User not found in Firestore", "warning");
//           }
//         } catch (err) {
...
//           <Text>Please ensure you are logged in.</Text>
//         </Card>
//       </PageContainer>
//     );
//   }
...
//       <Card>
//         <AvatarWrap>
//           <Avatar>{user.name?.charAt(0) || user.email?.charAt(0) || 'U'}</Avatar>
//         </AvatarWrap>

//         <Heading>{user.name || 'User'}</Heading>
//         <Text><Highlight>Email:</Highlight> {user.email}</Text>
...
//               />
//               <EditButton onClick={handlePhoneSave}>Save</EditButton>
//               <EditButton cancel onClick={() => setIsEditing(false)}>Cancel</EditButton>
//             </>
//           ) : (
//             <>
//               <Text style={{ margin: 0 }}>{phone || 'Not Provided'}</Text>
...
// Your styled components like PageContainer, Card, etc.
...
const UserProfile = () => {
      ...
      const fetchUser = async () => {
        try {
          ...
          if (userSnap.exists()) {
            ...
            Swal.fire("User Not Found", "No user document found with this ID.", "warning");
          }
        } catch (err) {
  ...
  const handlePhoneSave = async () => {
    ...
    try {
      ...
      Swal.fire("Error", "Could not update phone number. Please try again.", "error");
    }
  };
    ...
    return (
      <PageContainer>
        <Card>
          ...
          <Text>Please wait while we load your profile.</Text>
        </Card>
      </PageContainer>
    );
  }
  ...
  return (
    <PageContainer>
      <Card>
        <AvatarWrap>
          <Avatar>{user.name?.charAt(0)?.toUpperCase() || user.email?.charAt(0)?.toUpperCase() || 'U'}</Avatar>
        </AvatarWrap>
